(function(angular) {
    ...
    robotshop.factory('currentUser', function($rootScope) {
        var data = {
            uniqueid: '',
            user: {},
            cart: {
        ...
        try {
            var saved = JSON.parse(sessionStorage.getItem('stans-session'));
            if(saved && saved.uniqueid) angular.extend(data, saved);
        } catch(e) { /* Storage may be unavailable. */ }
        $rootScope.$watch(function() { return data; }, function() {
            try { sessionStorage.setItem('stans-session', JSON.stringify(data)); } catch(e) {}
        }, true);
        return data;
    });
    ...
    robotshop.controller('shopform', function($scope, $http, $location, $q, currentUser) {
    ...
    robotshop.controller('cartform', function($scope, $http, $location, currentUser) {
        ...
        function loadCart(id) {
            $http({
                ...
                if(cart.items.length && cart.items[cart.items.length - 1].sku == 'SHIP') {
                    $http({
        ...
        $scope.$watch(function() { return currentUser.uniqueid; }, function(id) {
            if(id) { $scope.data.uniqueid = id; loadCart(id); }
        });
    ...
    robotshop.controller('loginform', function($scope, $http, $location, currentUser) {
        $scope.data = {};
        $scope.data.registering = $location.search().mode === 'signup';
        $scope.data.name = '';
        $scope.data.email = '';
        $scope.data.password = '';
        ...
        $scope.register = function() {
            ...
            $http({
                ...
                currentUser.user = $scope.data.user;
                currentUser.uniqueid = $scope.data.user.name;
                // Preserve the guest bag when creating an account during shopping.
                $http.get('/api/cart/rename/' + oldId + '/' + currentUser.uniqueid)
                    .catch((e) => {
                        // No existing guest cart is a normal first-visit case.
                        if(e.status !== 404) console.log('ERROR moving cart', e);
                    });
            }).catch((e) => {
                console.log('ERROR', e);
                $scope.data.message = 'ERROR ' + e.data;
